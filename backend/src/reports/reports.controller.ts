...
export class ReportsController {
  ...
  summary(@CurrentUser() actor: CurrentUserPayload, @Query() query: ReportQueryDto) {
    return this.reportsService.getSalesSummary(requireTenantId(actor), query.period, parseReportDate(query.date));
  }

  @Get('products')
  products(@CurrentUser() actor: CurrentUserPayload, @Query() query: ReportQueryDto) {
    return this.reportsService.getProductBreakdown(requireTenantId(actor), query.period, parseReportDate(query.date));
  }

  @Get('waiters')
  waiters(@CurrentUser() actor: CurrentUserPayload, @Query() query: ReportQueryDto) {
    return this.reportsService.getWaiterPerformance(requireTenantId(actor), query.period, parseReportDate(query.date));
  }

  @Get('expenses')
  expenses(@CurrentUser() actor: CurrentUserPayload, @Query() query: ReportQueryDto) {
    return this.reportsService.getExpenseBreakdown(requireTenantId(actor), query.period, parseReportDate(query.date));
  }
}
